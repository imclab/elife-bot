...
class Templates(object):
  ...
  def get_email_templates_list(self):
    ...
    template_list.append("editor_publication_email.html")
    template_list.append("editor_publication_email.json")
  ...
  def get_s3_key_name(self, template_type, template_name):
    ...
    if(template_type == "email"):
      ...
      s3_key_name = "email" + delimiter + template_name
  ...
  def get_author_publication_email_body(self, author, article, elife, format = "html"):
    """
    ...
    if(self.email_templates_warmed is True):
      ...
      body = tmpl.render(author = author, article = article, elife = elife)
      return body
    else:
      return None
    
  def get_author_publication_email_headers(self, author, article, elife, format = "html"):
    """
    Given data objects, load the jinja environment,
    get the template, render it and return the
    email headers
    ...
    if(self.email_templates_warmed is True):
      ...
      headers_str = tmpl.render(author = author, article = article, elife = elife)
  ...
  def get_editor_publication_email_body(self, editor, article, elife, format = "html"):
    """
    Given data objects, load the jinja environment,
    get the template, render it and return the
    email body
    ...
    if(self.email_templates_warmed is True):
      ...
      jinja_env = self.get_jinja_env()
      if(format == "html"):
        template_name = "editor_publication_email.html"
      elif(format == "text"):
        template_name = "editor_publication_email.txt"
      tmpl = self.get_jinja_template(jinja_env, template_name)
      body = tmpl.render(editor = editor, article = article, elife = elife)
      return body
    else:
      return None
    
  def get_editor_publication_email_headers(self, editor, article, elife, format = "html"):
    """
    Given data objects, load the jinja environment,
    get the template, render it and return the
    email headers
    ...
    if(self.email_templates_warmed is True):
      ...
      jinja_env = self.get_jinja_env()
      tmpl = self.get_jinja_template(jinja_env, "editor_publication_email.json")
      headers_str = tmpl.render(editor = editor, article = article, elife = elife)
      headers = json.loads(headers_str)
      # Add the email format as specified
      headers["format"] = format
      return headers
